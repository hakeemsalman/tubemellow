...
export default function Tooltip({tooltip, children, direction = 'left-12 top-0'}: TooltipProps ) {
  const [isOpen, setIsOpen] = useState<boolean>(false);
  return (
    <div className="relative">
      <span  onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)} className="flex justify-center">{children}</span>
